...
public class GameTree {
    ...
    int findValue() {
        ...
        return this.root == null
            ? Integer.MIN_VALUE
            : findValueHelper(this.root, true);
    }

    // helper function
    int findValueHelper(TreeNode node, boolean isPlayerOne) {
        if (node.leaf) {
            return node.value;
        }
        ...
        if (isPlayerOne) {
            ...
            for (TreeNode child : node.children) {
                score = Math.max(score, findValueHelper(child, false));
            }
            node.value = score;
        } else {
            int score = Integer.MAX_VALUE;
            for (TreeNode child : node.children) {
                score = Math.min(score, findValueHelper(child, true));
            }
            node.value = score;
        }
